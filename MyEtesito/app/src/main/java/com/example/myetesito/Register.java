...
import com.example.myetesito.Login;
...
public class Register extends AppCompatActivity {
    ...
    private static final String TAG = "MyEtesito";
...
/*
        ...
        if ("".equals(Name.getText().toString()) || "".equals(firstName.getText().toString()) ||
            ...
            if (Name.length() > 22 || firstName.length() > 15 || lastName.length() > 15 || userName.length() > 20 ||
                ...
                if (firstName.length() > 15) {
                    mensaje = "Apellido Paterno Muy Largo";
                }
                if (lastName.length() > 15) {
                    mensaje = "Apellido Materno Muy Largo";
                }
                if (userName.length() > 20) {
                    mensaje = "Nombre de Usuario Muy Largo";
                }
                if (TipoCorreo == false) {
                    mensaje = "Correo Invalido, Intente con los dominios @gmail.com, @hotmail.com, @outlook.com";
                }
                if (Mail.length() > 25) {
                    mensaje = "Correo Muy Largo";
                }
                if (Age.length() > 2) {
                    mensaje = "Edad Invalida, Intente con una edad mas corta";
                }
                if (Number.length() != 10) {
                    mensaje = "Numero Invalido, Intente con un numero de 10 digitos";
                }
                ...
                try {
                    //le metemos sha1 a la contraseña
                    Sha1 digest = new Sha1();
                    byte[] txtByte = digest.createSha1(userName.getText().toString() + Password.getText().toString());
                    String Sha1Password = digest.bytesToHex(txtByte);
                    String ValorPassword = Sha1Password;

                    //primero hacemos nuestro relajo de sql
                    //comparamos los valores de los booleanos para obtener la respuesta
                    String ValorGenero = " ";
                    String ValorTipo = " ";
                    if (Gender1.isChecked() == true) {
                        ValorGenero = "ConLetra";
                    } else {
                        ValorGenero = "SinLetra";
                    }
                    if (Type1.isChecked() == true) {
                        ValorTipo = "Mexicana";
                    } else {
                        ValorTipo = "Extranjera";
                    }
                    //para meterle los datos del usuario a la bd
                    BDCuentas bdCuentas = new BDCuentas(Register.this);
                    long id = bdCuentas.insertarUsuario(Name.getText().toString(), userName.getText().toString(), firstName.getText().toString(), lastName.getText().toString(), Mail.getText().toString(), Age.getText().toString(), Number.getText().toString(), ValorGenero, ValorTipo, ValorPassword);
                    ...
                    if (id > 0) {
                        ...
                        Toast.makeText(this, "REGISTRO SIN GUARDADDO AAA", Toast.LENGTH_LONG).show();
                    }
    ...
    public void CrearBD (View v){
        BDHelper bdHelper = new BDHelper(Register.this);
        SQLiteDatabase bd = bdHelper.getWritableDatabase();
        if (bd != null) {
            Toast.makeText(Register.this, "BASE DE DATOS CREADA", Toast.LENGTH_SHORT).show();
        } else {
            Toast.makeText(Register.this, "HELP! ERROR EN LA BASE DE DATOS", Toast.LENGTH_SHORT).show();
        }
    }
    public void Volver (View v){
        Intent intent = new Intent (Register.this, Login.class);
